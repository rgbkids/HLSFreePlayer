...
public class MediaPlayerCtrl : MonoBehaviour
{
	...
	private unsafe bool Call_Load(string strFileName, int iSeek)
	{
        ...
        pFormatContext->max_analyze_duration = 500;
        pFormatContext->probesize = 100000;
	...
	private void ThreadUpdate()
	{
		...
		while (true)
		{
			...
			if (listVideo != null)
			{
                if (m_queuePacketVideo.Count < 30)
                    ReadPacket();

                while (listVideo.Count >5 || bEnd == true )
				{
                    if (m_queuePacketVideo.Count < 30)
                    {
                        ReadPacket();
	...
	private void UpdateVideo()
	{
        ...
        if(m_queuePacketVideo.Count > 20 || bEndCheck == true)
        {
            bBuffering = false;
